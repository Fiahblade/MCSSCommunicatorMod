...
public class HTTPClient {
    ...
    public HTTPClient(String url, Logger logger) {
        this.defaultUrl = "http://localhost:9696";
        this.logger = logger;
    ...
    private boolean setUrl(String url) {
        try {
            ...
            logger.error(String.format("Failed to parse url form config File. Using default value '(%s)'", defaultUrl));
            return false;
        }
        return true;
    }
    ...
    public void post(String path, String json) {
        ...
        try {
            ...
            logger.fatal(String.format("Failed to post data to mcss: ", ex.getMessage()));
        }
    }
}
